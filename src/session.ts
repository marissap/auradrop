...
export class Session implements DurableObject {
    ...
    async fetch(req: Request): Promise<Response> {
        ...
        // do i want to sendd full history on connect so late joiners can read?? nah
        // server.send(JSON.stringify({ type: "history", messages: this.history }));

        console.log("Session fetch, sessionId from URL:", url.pathname, "connections:", this.connections.size);

        this.broadcast({
            type: "joined", hash: userHash, name: userName,
        ...
        server.addEventListener("close", () => {
            ...
            this.broadcast({
                type: "left", hash: userHash, name: userName,
                participants: this.connections.size
            });
        });
